...
{
    public sealed class BaseEdgeDragHelper : EdgeDragHelper
    {
        ...
        private Dictionary<Node, List<NodePort>> compatiblePorts = new Dictionary<Node, List<NodePort>>();
        ...
        public override bool HandleMouseDown(MouseDownEvent evt)
        {
            ...
            foreach (NodePort port in graphView.GetCompatiblePorts(draggedPort, nodeAdapter))
            {
                compatiblePorts.TryGetValue(port.nodeView, out var portList);
                if (portList == null)
                    portList = compatiblePorts[port.nodeView] = new List<NodePort>();
                portList.Add(port);
            }
        ...
        Rect GetPortBounds(Node node, int index, List<NodePort> portList)
        {
